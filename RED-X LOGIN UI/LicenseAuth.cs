...
namespace LicenseAuth
{
    public class api
    {
        ...
        /// <summary>
        /// Set up your application credentials in order to use LicenseAuth
        /// </summary>
        ...
        public api(string name, string ownerid, string secret, string version, string path = null)
        {
            ...
            {
                Process.Start("https://licenseauth.cloud/app/");
                Thread.Sleep(2000);
                error("Application not setup correctly. Get support for setup or setup correctly.");
                Environment.Exit(0);
            }
        ...
        bool initialized;
        /// <summary>
        /// Initializes the connection with LicenseAuth in order to use any of the functions
        /// </summary>
        public void init()
        {
            ...
            if (response == "LicenseAuth_Invalid")
            {
        ...
        /// <summary>
        /// Checks if LicenseAuth is been Initalized
        /// </summary>
        public void CheckInit()
        {
            if (!initialized)
            {
                error("You must run the function LicenseAuthApp.init(); first");
                Environment.Exit(0);
            }
        }
        ...
        /// <summary>
        /// Use Buttons from LicenseAuth Customer Panel
        /// </summary>
        ...
        /// <summary>
        /// LicenseAuth acts as proxy and downlods the file in a secure way
        /// </summary>
        ...
        public static void LogEvent(string content)
        {
            ...
            string logDirectory = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.CommonApplicationData), "LicenseAuth", "debug", exeName);
        ...
        private static string req(NameValueCollection post_data)
        {
            try
            {
                using (WebClient client = new WebClient())
                {
                    ...
                    var raw_response = client.UploadValues("https://licenseauth.cloud/api/1.3/", post_data);
            ...
            catch (WebException webex)
            {
                ...
                switch (response.StatusCode)
                {
                    case (HttpStatusCode)429: // client hit our rate limit
                        ...
                        Environment.Exit(0);
                        return "";
                    default: // site won't resolve. you should use licenseauth.cloud domain since it's not blocked by any ISPs
